...
async function executeBrowserAction(action, params) {
  ...
  try {
    switch (action) {
      ...
      case 'scroll':
        // Sanitize args: undefined is not serializable by Chrome scripting API
        const scrollDir = params.direction || 'down';
        const scrollAmt = typeof params.amount === 'number' ? params.amount : 0;
...
\`\`\`tool_call
{
  "action": "click|type|hover|scroll|navigate|wait|record|screenshot",
  "params": {
    ...
    "amount": "Milliseconds to wait (if action=wait), or pixels to scroll (if action=scroll). Must be a number, not undefined.",
