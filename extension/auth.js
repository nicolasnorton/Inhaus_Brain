document.addEventListener('DOMContentLoaded', () => {
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        // Timeout fallback: if background never responds, re-enable the button
        const loginTimeout = setTimeout(() => {
            loginBtn.disabled = false;
            loginBtn.innerText = "Sign In";
            errorMessage.innerText = "Login timed out. Please try again.";
        }, 15000);
    ...
    function handleLoginResponse(response) {
        ...
        if (response && response.success) {
            ...
            errorMessage.innerText = response?.error || "Authentication failed. Check your credentials.";
        }
    }
});
