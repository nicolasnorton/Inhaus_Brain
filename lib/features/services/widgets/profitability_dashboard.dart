...
class ProfitabilityDashboard extends StatelessWidget {
  final List<Service> services;

  const ProfitabilityDashboard({super.key, required this.services});

  @override
  Widget build(BuildContext context) {
    if (services.isEmpty) {
      return const Center(child: Text("No services to analyze", style: TextStyle(color: Colors.white54)));
    }

    final totalRevenue = services.fold<double>(0, (sum, s) => sum + s.basePrice);
    final totalCosts = services.fold<double>(0, (sum, s) => sum + s.deliveryCost);
  ...
  Widget _buildRankingTable(BuildContext context) {
    final sortedServices = List<Service>.from(services)..sort((a, b) => b.margin.compareTo(a.margin));
        ...
        rows: sortedServices.map((s) {
          ...
          return DataRow(
            cells: [
              ...
              DataCell(Text("\$${s.profit.toStringAsFixed(0)}", style: const TextStyle(color: Colors.blue))),
              DataCell(Text("${s.margin.toStringAsFixed(1)}%", style: TextStyle(color: isHealthy ? Colors.green : Colors.orange, fontWeight: FontWeight.bold))),
              DataCell(
                Container(
